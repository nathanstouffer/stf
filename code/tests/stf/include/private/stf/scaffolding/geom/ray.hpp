...
struct dist_and_dist_squared
{
    ...
    void verify(size_t const i) const
    {
        ...
        ASSERT_NEAR(std::sqrt(dist_squared), stf::geom::dist(beam, point), stf::math::constants<T>::tol)
            << info(i) << "failed distance between ray and point";
        ASSERT_NEAR(std::sqrt(dist_squared), stf::geom::dist(point, beam), stf::math::constants<T>::tol)
        ...
        ASSERT_NEAR(dist_squared, stf::geom::dist_squared(beam, point), stf::math::constants<T>::tol)
            << info(i) << "failed distance squared from ray to vec";
        ASSERT_NEAR(dist_squared, stf::geom::dist_squared(point, beam), stf::math::constants<T>::tol)
