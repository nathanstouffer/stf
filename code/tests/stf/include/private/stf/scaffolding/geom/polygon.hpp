...
struct contains
{
    ...
    void verify(size_t const i) const
    {
        ASSERT_EQ(contained, polygon.contains(query, boundary_type))
            << info(i) << "failed polygon::contains";
    }
};
...
struct distances
{
    ...
    void verify(size_t const i) const
    {
        ASSERT_EQ(signed_distance, polygon.signed_dist(query))
            << info(i) << "failed polygon::signed_dist";
        ASSERT_EQ(signed_distance, stf::geom::signed_dist(polygon, query))
            << info(i) << "failed geom::signed_dist";
        ASSERT_EQ(signed_distance, stf::geom::signed_dist(query, polygon))
            << info(i) << "failed geom::signed_dist";
        ...
        ASSERT_NEAR(dist * dist, stf::geom::dist_squared(polygon, query),
                    stf::math::constants<T>::tol)
            << info(i) << "failed geom::dist_squared";
        ASSERT_NEAR(dist * dist, stf::geom::dist_squared(query, polygon),
                    stf::math::constants<T>::tol)
            << info(i) << "failed geom::dist_squared";
    }
};
