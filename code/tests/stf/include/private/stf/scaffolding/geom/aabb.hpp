...
struct intersects
{
    ...
    void verify(size_t const i) const
    {
        ASSERT_EQ(intersect, lhs.intersects(rhs))
            << info(i) << "failed lhs -> rhs intersection test (member)";
        ASSERT_EQ(intersect, rhs.intersects(lhs))
            << info(i) << "failed rhs -> lhs intersection test (member)";

        ASSERT_EQ(intersect, alg::intersects(lhs, rhs))
            << info(i) << "failed lhs -> rhs intersection test (free)";
        ASSERT_EQ(intersect, alg::intersects(rhs, lhs))
            << info(i) << "failed rhs -> lhs intersection test (free)";
    }
};
...
struct contains
{
    ...
    void verify(size_t const i) const
    {
        ASSERT_EQ(contained, lhs.contains(rhs))
            << info(i) << "failed lhs -> rhs containment test (member)";
        ASSERT_EQ(contained, alg::contains(lhs, rhs))
            << info(i) << "failed lhs -> rhs containment test (free)";
    }
};
...
struct dist_and_dist_squared
{
    ...
    void verify(size_t const i) const
    {
        ASSERT_EQ(std::sqrt(dist_squared), box.dist(point))
        ...
        ASSERT_EQ(dist_squared, box.dist_squared(point))
