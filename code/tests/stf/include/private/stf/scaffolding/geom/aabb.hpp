...
namespace stf::scaffolding::geom::aabb
{
    ...
    struct dist_and_dist_squared
    {
        ...
        void verify(size_t const i) const
        {
            ...
            ASSERT_EQ(std::sqrt(dist_squared), stf::geom::dist(box, point)) << info(i) << "failed distance from aabb to vec";
            ASSERT_EQ(std::sqrt(dist_squared), stf::geom::dist(point, box)) << info(i) << "failed distance from vec to aabb";

            ASSERT_EQ(dist_squared, box.dist_squared(point)) << info(i) << "failed distance squared from aabb to vec";
            ASSERT_EQ(dist_squared, stf::geom::dist_squared(box, point)) << info(i) << "failed distance squared from aabb to vec";
            ASSERT_EQ(dist_squared, stf::geom::dist_squared(point, box)) << info(i) << "failed distance squared from vec to aabb";
        }
    };
