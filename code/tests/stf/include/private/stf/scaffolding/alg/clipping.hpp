...
struct segment
{
    ...
    void verify(size_t const i) const
    {
        ...
        ASSERT_EQ(accept, stf::alg::clip(box, clipped))
            << info(i) << "Failed to accept/reject segment";
        ...
        ASSERT_EQ(accept, stf::alg::clip(box, flipped))
            << info(i) << "Failed to accept/reject flipped segment";
