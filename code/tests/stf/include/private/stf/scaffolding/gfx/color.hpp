...
struct component_to_hex
{
    ...
    void verify(size_t const i) const
    {
        ASSERT_EQ(expected, stf::gfx::rgba::to_hex(x))
            << info(i) << "Failed to compute hex correctly";
    }
};
...
struct component_from_hex
{
    ...
    void verify(size_t const i) const
    {
        ASSERT_EQ(expected, stf::gfx::rgba::from_hex(hex, shift))
            << info(i) << "Failed to compute value correctly";
    }
};
...
struct rgba_hex_conversion
{
    ...
    void verify(size_t const i) const
    {
        ...
        ASSERT_EQ(color, stf::gfx::rgba::from_hex_rgba(hex_rgba))
            << info(i) << "failed conversion from rgba";
        ASSERT_EQ(color, stf::gfx::rgba::from_hex_abgr(hex_abgr))
            << info(i) << "failed conversion from abgr";
        ASSERT_EQ(color, stf::gfx::rgba::from_hex_argb(hex_argb))
            << info(i) << "failed conversion from argb";
    }
};
