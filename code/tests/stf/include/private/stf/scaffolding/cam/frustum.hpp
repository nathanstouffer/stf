...
struct contains
{
    ...
    {
        ASSERT_EQ(contains, frustum.contains(box))
            << info(i) << "Failed to compute frustum::contains(aabb)";
...
struct intersects_fast
{
    ...
    void verify(size_t const i) const
    {
        ASSERT_EQ(intersects, frustum.intersects_fast(box))
            << info(i) << "Failed to compute frustum::intersects_fast";
...
struct intersects
{
    ...
    void verify(size_t const i) const
    {
        ASSERT_EQ(intersects, frustum.intersects(box))
            << info(i) << "Failed to compute frustum::intersects aabb";
        ...
        ASSERT_EQ(intersects, frustum.intersects(obb))
            << info(i) << "Failed to compute frustum::intersects obb";
