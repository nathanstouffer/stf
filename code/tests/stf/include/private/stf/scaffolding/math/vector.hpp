...
struct length
{
    ...
    void verify(size_t const i) const { ASSERT_EQ(expected, initial.length()) << info(i) << "Failed initial.length()"; }
};
...
struct hadamard_divide
{
    ...
    void verify(size_t const i) const { ASSERT_EQ(expected, lhs / rhs) << info(i) << "Failed lhs / rhs"; }
};
