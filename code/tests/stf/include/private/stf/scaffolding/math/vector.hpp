...
struct equality
{
    ...
    void verify(size_t const i) const
    {
        if (equal)
        {
            ...
            ASSERT_TRUE(stf::math::equ(lhs, rhs, stf::math::constants<T>::zero))
                << info(i) << "failed exact positive assertion";
            ASSERT_FALSE(stf::math::neq(lhs, rhs, stf::math::constants<T>::zero))
                << info(i) << "failed exact negative assertion";
        }
        else
        {
            ASSERT_NE(lhs, rhs) << info(i) << "failed positive assertion";
            ASSERT_FALSE(lhs == rhs) << info(i) << "failed negative assertion";
            ASSERT_TRUE(stf::math::neq(lhs, rhs, stf::math::constants<T>::zero))
                << info(i) << "failed exact positive assertion";
            ASSERT_FALSE(stf::math::equ(lhs, rhs, stf::math::constants<T>::zero))
                << info(i) << "failed exact negative assertion";
        }
    }
};
...
struct cast
{
    ...
    void verify(size_t const index) const
    {
        ...
        for (size_t i = 0; i < N; ++i)
        {
            ASSERT_EQ(static_cast<U>(initial[i]), result[i])
                << info(index) << "Failed to cast properly";
        }
    }
};
...
struct hadamard_multiply
{
    ...
    void verify(size_t const i) const
    {
        ASSERT_EQ(expected, lhs * rhs) << info(i) << "Failed lhs * rhs";
        ASSERT_EQ(expected, rhs * lhs) << info(i) << "Failed rhs * lhs";
        ASSERT_EQ(expected, stf::math::hadamard(lhs, rhs))
            << info(i) << "Failed stf::math::hadamard(lhs, rhs)";
        ASSERT_EQ(expected, stf::math::hadamard(rhs, lhs))
            << info(i) << "Failed stf::math::hadamard(rhs, lhs)";
    }
};
