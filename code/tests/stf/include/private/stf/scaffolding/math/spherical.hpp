...
struct conversion
{
    ...
    {
        ASSERT_FLOAT_EQ(degrees, stf::math::to_degrees(radians))
            << info(i) << "Failed to convert radians to degrees";
        ASSERT_FLOAT_EQ(radians, stf::math::to_radians(degrees))
            << info(i) << "Failed to convert degrees to radians";
    }
};
...
struct closest_equiv_angle
{
    ...
    void verify(size_t const i) const
    {
        ...
        ASSERT_NEAR(expected, stf::math::closest_equiv_angle(phi, theta),
...
struct unit_vec2
{
    ...
    void verify(size_t const i) const
    {
        ASSERT_EQ(expected, stf::math::unit_vector(theta))
            << info(i) << "Failed to compute unit vector";
    }
};
