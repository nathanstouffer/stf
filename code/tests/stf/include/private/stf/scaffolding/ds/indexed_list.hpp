...
inline void verify(std::vector<element_t> const& elements, list_t const& list)
{
    ...
    {
        ASSERT_EQ(elements[i].key, element.key)
            << "Failed key comparison for element with index '" << i << "'";
        ASSERT_EQ(elements[i].entry, element.entry)
            << "Failed entry comparison for element with index '" << i << "'";
        ++i;
    }
}
