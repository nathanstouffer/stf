...
TEST(polyline2, length)
{
    std::vector<scaffolding::geom::polyline::length<float, 2>> tests = {
        ...
        {stff::polyline2({stff::vec2(0), stff::vec2(1), stff::vec2(2, 0)}),
         2 * stff::constants::sqrt_two},
        {stff::polyline2({stff::vec2(0), stff::vec2(1), stff::vec2(2, 0), stff::vec2(1, -1)}),
         3 * stff::constants::sqrt_two},
    };
...
TEST(polyline2, interpolate)
{
    std::vector<scaffolding::geom::polyline::interpolate<float, 2>> tests = {
        ...
        {stff::polyline2({stff::vec2(0), stff::vec2(1), stff::vec2(2, 0)}), 0.75,
         stff::vec2(1.5, 0.5)},
    };
    scaffolding::verify(tests);
}
