...
TEST(frustum, contains)
{
    ...
    stfd::scamera camera(stfd::vec3(0), stfd::constants::pi_halves, stfd::constants::pi_halves, 1.0,
                         100.0, 1.0, stfd::constants::pi_halves);
    stfd::frustum frustum(camera);

    double eps = 1.0E-14; // slightly inset the box because some compilers have slight differences
                          // in floating point precision
    std::vector<scaffolding::cam::frustum::contains<double>> tests = {
        {frustum, stfd::aabb3(stfd::vec3(-1 + eps, 1 + eps, -1 - eps), 2 - 2.0 * eps),
         true}, // slightly inset the box
        ...
        {frustum, stfd::aabb3(stfd::vec3(110, 90, -50), 100),
         false}, // entirely outside the frustum
    };
...
TEST(frustum, intersects_fast)
{
    // true intersections
    {
        // camera with the eye at the origin that looks along the +y axis with the +z axis is the up
        // vector
        stfd::scamera camera(stfd::vec3(0), stfd::constants::pi_halves, stfd::constants::pi_halves,
                             1.0, 100.0, 1.0, stfd::constants::pi_halves);
        stfd::frustum frustum(camera);
    ...
    // false positives
    {
        // camera with the eye at the origin that looks along the -z axis with vec3(1, 1, 0)
        // defining the up direction
        stfd::scamera camera(stfd::vec3(0), stfd::constants::pi_fourths, stfd::constants::pi, 1.0,
                             100.0, 1.0, stfd::constants::pi_halves);
        stfd::frustum frustum(camera);
