...
struct mtx final
{
    ...
    /**
     * @brief Construct from an N-dimensional vector -- intializes the diagonal to the value of the
     * vector
     * @param [in] diagonal
     */
    ...
    /**
     * @brief Construct from an NxN-dimensional vector -- an explicit column-major initialization of
    ...
    T determinant() const
    {
        ...
        else
        {
            ...
            for (size_t i = 0; i < N; ++i)
            {
                ...
                det += (even) ? expansion
                              : -expansion; // even i => sign is positve; odd i => sign is negative
            }
            return det;
        }
    }
     ...
     * @return A reference to @p this
     */
    inline mtx& scale_by(vec<T, N - 1> const& scalars)
    {
        return scale_by(vec<T, N>(scalars, T(1)));
    }
     ...
     * @return The resulting transformation matrix
     */
    inline mtx scaled_by(vec<T, N - 1> const& scalars) const
    {
        return mtx(*this).scale_by(vec<T, N>(scalars, T(1)));
    }
     ...
     * @return A reference to @p this
     */
    inline mtx& translate_by(vec<T, N - 1> const& scalars)
    {
        return (*this) *= mtx::translate(scalars);
    }
     ...
     * @return The resulting transformation matrix
     */
    inline mtx translated_by(vec<T, N - 1> const& scalars) const
    {
        return mtx(*this).translate_by(scalars);
    }
    ...
    {
        std::memcpy(static_cast<void*>(fill), static_cast<void*>(values), sizeof(T) * D);
    }
 ...
 */
template <typename T>
inline mtx4<T> scale(vec<T, 3> const& scalars)
{
    return mtx4<T>::scale(scalars);
}
 ...
 */
template <typename T>
inline mtx4<T> scale_x(T const scalar)
{
    return mtx4<T>::scale(vec<T, 3>(scalar, T(1), T(1)));
}
 ...
 */
template <typename T>
inline mtx4<T> scale_y(T const scalar)
{
    return mtx4<T>::scale(vec<T, 3>(T(1), scalar, T(1)));
}
 ...
 */
template <typename T>
inline mtx4<T> scale_z(T const scalar)
{
    return mtx4<T>::scale(vec<T, 3>(T(1), T(1)), scalar);
}
 ...
 */
template <typename T>
inline mtx4<T> translate(vec<T, 3> const& scalars)
{
    return mtx4<T>::translate(scalars);
}
 ...
 */
template <typename T>
inline mtx4<T> translate_x(T const scalar)
{
    return mtx4<T>::translate(vec<T, 3>(scalar, T(0), T(0)));
}
 ...
 */
template <typename T>
inline mtx4<T> translate_y(T const scalar)
{
    return mtx4<T>::translate(vec<T, 3>(T(0), scalar, T(0)));
}
 ...
 */
template <typename T>
inline mtx4<T> translate_z(T const scalar)
{
    return mtx4<T>::translate(vec<T, 3>(T(0), T(0)), scalar);
}
 ...
 */
template <typename T>
inline mtx4<T> rotate_xyz(T const x, T const y, T const z)
{
 ...
 */
template <typename T>
inline mtx4<T> rotate_xzy(T const x, T const z, T const y)
{
    return (rotate_x(x) *= rotate_z(z)) *= rotate_y(y);
}
 ...
 */
template <typename T>
inline mtx4<T> rotate_yxz(T const y, T const x, T const z)
{
    return (rotate_y(y) *= rotate_x(x)) *= rotate_z(z);
}
 ...
 */
template <typename T>
inline mtx4<T> rotate_yzx(T const y, T const z, T const x)
{
    return (rotate_y(y) *= rotate_z(z)) *= rotate_x(x);
}
 ...
 */
template <typename T>
inline mtx4<T> rotate_zxy(T const z, T const x, T const y)
{
 ...
 */
template <typename T>
inline mtx4<T> rotate_zyx(T const z, T const y, T const x)
{
 ...
 */
template <typename T>
inline mtx4<T> view(vec3<T> const& eye, vec3<T> const& look, vec3<T> const& right,
                    vec3<T> const& up)
{
    vec3<T> v = -look;
    vec3<T> r = right;
 ...
 * @note Angles @p delta_phi and @p delta_theta are spherical coordinates measured with @p focus as
 * the origin
 * @note @p right must be a unit vector
 * @return The transformation matrix
 */
template <typename T>
inline mtx4<T> orbit(vec<T, 3> const& focus, vec<T, 3> const& right, T const delta_phi,
                     T const delta_theta)
{
