 ...
 * @tparam N Dimension
 *
 * @todo Possibly use the CRTP to reduce verbosity -- just make sure to test performance/memory
 * layout implications. Unfortunately there is a lot of duplication between the generic vector class
 * and the specializations when the dimension is specialized for N = 2, 3, 4. This could be avoided
 * by pairing Curiously Recurring Template Pattern with some casting to the derived class type. But
 * I opted for simplicity even though it involves more duplication. To reduce some of the unwanted
 * duplication, many of the member functions call through to templated functions that operate
...
struct vec final
{
     ...
     *
     * @p prefix is used to fill the first N - 1 dimensions and @p suffix is used as the value for
     * the N dimension
     *
     * @param [in] prefix
     * @param [in] suffix
     ...
     */
    inline vec projected_on(vec const& rhs) const
    {
        T scalar = dot(rhs) / rhs.dot(rhs);
        return scalar * rhs;
    }
...
#    endif
    ...
    /**
     * @brief A union of a raw scalar array of size 3 and various structs allowing granular member
     * access
     */
    union
    {
...
#    endif
    ...
    /**
     * @brief A union of a raw scalar array of size 4 and various structs allowing granular member
     * access
     */
    union
    {
...
#    endif
     ...
     * @param [in] xyzw
     */
    explicit constexpr vec(const_array_t const& xyzw)
        : x(xyzw[0])
        , y(xyzw[1])
        , z(xyzw[2])
        , w(xyzw[3])
...
{
    return vec3<T>(cross(vec2<T>(lhs.y, lhs.z), vec2<T>(rhs.y, rhs.z)),
