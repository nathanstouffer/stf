 ...
 * reduce some of the unwanted duplication, many of the member functions call through to templated functions that
 * operate directly on the underlying raw pointers
 */
...
#    endif
    ...
    inline T dot(vec const& rhs) const
    {
        return raw::dot<T, 2>(values, rhs.values); // NOLINT(cppcoreguidelines-pro-bounds-array-to-pointer-decay)
    }
