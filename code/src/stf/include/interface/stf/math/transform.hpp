...
/**
 * @file transform.hpp
 * @brief A file containing a templated functionality dealing with named transformations of
...
{
    math::vec3<T> const c =
        dot(axis, point) * axis; // the center of our circle about the rotation axis
 ...
 */
template <typename T>
inline math::vec3<T> orbit(math::vec3<T> const& point, math::vec3<T> const& focus,
                           math::vec3<T> const& right, T const delta_phi, T const delta_theta)
{
