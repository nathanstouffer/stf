...
{
/**
 * @brief An circular interval class representing a counterclockwise connected component of the
 * circle.
...
struct cinterval final
{
    ...
    {
        return (a <= b) ? interval<T>::contains(a, b, x, type)
                        : !interval<T>::contains(b, a, x, complement(type));
    }
