 ...
 *
 * @note Because this function assumes a domain of [0, 1], the derivatives @p m0 and @p m1 must be scaled by the length
 * of the actual interval between anchor points
 *
 * @tparam T Number type (eg float)
...
inline math::vec<T, N> cubic_bezier(math::vec<T, N> const a, math::vec<T, N> const b, math::vec<T, N> const c,
                                    math::vec<T, N> const d, T const t)
{
    math::vec<T, N> result;
    for (size_t i = 0; i < N; ++i)
    {
