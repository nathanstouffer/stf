...
class gradient final
{
  public:
     ...
     * @param [in] stops
     * @param [in] period The period used when sampling the color -- 0 signifies that there is no
     * period
     */
    gradient(std::vector<stop> const& stops, float period = math::constants<time_t>::zero)
        : m_stops(stops)
        , m_period(period)
    {
    }
    ...
    rgba sample(time_t const x) const
    {
        ...
        if (m_stops.empty())
        {
            return rgba::from_hex_argb(0xFF000000);
        }
        ...
        if (m_period != math::constants<time_t>::zero)
        {
            ...
            else
            {
                ...
                size_t i = 0;
                while (!(m_stops[i].x <= x && x <= m_stops[i + 1].x))
                {
                    ++i;
                }
