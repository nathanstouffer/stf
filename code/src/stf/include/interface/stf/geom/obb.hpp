...
public:
    ...
    explicit obb(math::mtx<T, N> const& rotation)
        : obb(vec_t(math::constants<T>::zero), math::to_basis(rotation), vec_t(math::constants<T>::neg_inf))
    {
    }
    ...
    explicit obb(aabb<T, N> const& aabb)
        : obb(aabb.center(), math::canonical_basis<T, N>(), math::constants<T>::half * aabb.diagonal())
    {
    }
    ...
    vec_t extremity(vec_t const& direction) const
    {
        ...
        {
            T sign = (math::dot(direction, m_basis[i]) > math::constants<T>::zero) ? math::constants<T>::one
