...
class holygon final
{
  ...
  public:
     ...
     * @param [in] holes
     */
    holygon(polygon_t const& hull, std::vector<polygon_t> const& holes)
        : m_hull(hull)
        , m_holes(holes)
    {
    }
    ...
    T signed_area() const
    {
        ...
        T positive =
            std::abs(initial); // use positive area so we can always remove the area of the holes
