...
struct aabb final
{
  ...
  public:
     ...
     * @return A reference to @p this
     */
    aabb& translate(vec_t const& delta)
    {
        min += delta;
        max += delta;
        return *this;
  ...
  public:
    ...
    static aabb nothing()
    {
        return aabb(vec_t(math::constants<T>::pos_inf), vec_t(math::constants<T>::neg_inf));
    }
    ...
    static aabb unit()
    {
        return aabb(vec_t(math::constants<T>::zero), vec_t(math::constants<T>::one));
    }
