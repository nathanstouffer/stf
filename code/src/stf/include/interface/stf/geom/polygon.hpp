...
class polygon final
{
  ...
  public:
    ...
    /**
     * @brief Construct from an array of vertices -- there is an implicit edge from the last to the
     * first vertex
     * @param [in] vertices
     */
    polygon(std::vector<vec_t> const& vertices)
        : m_vertices(vertices)
     ...
     */
    inline segment2<T> edge(size_t i) const
    {
        return segment2<T>(m_vertices[i], m_vertices[(i + 1) % m_vertices.size()]);
    }
     ...
     */
    inline void write(size_t i, vec_t const& vertex)
    {
        m_vertices[i] = vertex;
     ...
     * @param [in] close Indicates whether the returned boundary should be 'closed' (by adding the
     * first point to the end) or if it should be left open
     ...
     * Computed using the trapezoid formula for polygon area on
    ...
    bool contains(vec_t const& query, boundary_types const type) const
    {
        ...
        {
            geom::segment2<T> seg = edge(i);
            if (geom::dist_squared(seg, query) ==
                math::constants<T>::zero) // early out if the point is on the boundary
            {
            ...
            else if ((seg.a.y > query.y) != (seg.b.y > query.y)) // test if the y-range is relevent
            {
                ...
                else if (query.x <= x_range.b) // make sure the x-range is relevant
                {
                    ...
                    {
                        // we compute the x coordinate of the pair (x, query.y) that is on the line
                        // defined by seg if the ray begins before that coordinate, then the ray
