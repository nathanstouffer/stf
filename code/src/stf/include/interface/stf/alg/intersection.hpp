...
namespace stf::alg
{
    ...
    bool intersect(geom::polygon<T> const& polygon, geom::aabb2<T> const& aabb)
    {
        ...
        {
            if (aabb.contains(polygon[0]) || polygon.contains(aabb.vertex(0), boundary_types::CLOSED))  // if a point from either is contained in the other, they certainly intersect
            {
                return true;
            }
            else
