                           ...
                           bool const add_end)
{
    ...
    {
        // the number of vertices we add to the interior of the segment
        size_t count = static_cast<size_t>(std::ceil(length / max_len - math::constants<T>::one));
 ...
 * @param [in] max_len The maximum segment length allowed in the returned sequence of vertices
 * @param [in] loop Whether or not the polyline should be closed with an additional segement from the back to the front
