...
bool clip(geom::aabb2<T> const& box, geom::segment2<T>& seg)
{
    ...
    // the official algorithm uses an infinite loop here, but there are certain inputs with
    // floating-point precision issues that cause the loop to never terminate in practice. 4 is the
    // maximum number of iterations that should ever occur
...
std::vector<geom::polyline2<T>> clip(geom::aabb2<T> const& box, geom::polyline2<T> const& polyline)
{
    ...
    for (size_t i = 0; i + 1 < polyline.size(); ++i)
    {
        ...
        if (clip(box, edge)) // segment intersects the box => test how to add the segment to the
                             // clipped result
        {
            ...
            else if (sub_polyline.back() ==
                     edge.a) // this segment should be added on to the existing polyline => only add
                             // the second point
            {
                sub_polyline.push_back(edge.b);
            }
            else // this segment is part of a new polyline => store the old polyline and start a new
                 // one
            {
        ...
        else if (!sub_polyline.is_empty()) // segment does not intersect the box => immediately add
                                           // the current sub_polyline to the result
        {
            clipped.push_back(sub_polyline);
            sub_polyline.clear();
        }
