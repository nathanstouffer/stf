...
{
    math::vec2<T> const& a = lhs.a;
    math::vec2<T> const& b = lhs.b;
    math::vec2<T> const& c = rhs.a;
    math::vec2<T> const& d = rhs.b;

    // compute which side of the lhs each point is on
    T const c_side = math::orientation(a, b, c);
    T const d_side = math::orientation(a, b, d);

    if (c_side == math::constants<T>::zero &&
        d_side == math::constants<T>::zero) // if all four points are colinear
    {
    ...
    else // the general case where not all four points are colinear
    {
        ...
        return (c_side * d_side <= math::constants<T>::zero) &&
               (a_side * b_side <= math::constants<T>::zero);
    }
}
...
bool intersects(geom::segment2<T> const& segment, geom::aabb2<T> const& aabb)
{
    ...
    if (aabb.contains(segment.a) || aabb.contains(segment.b))
    {
        return true;
    }

    // both points are outside the box => an intersection can only occur if the segment crosses a
    // diagonal
...
bool intersects(geom::polyline2<T> const& polyline, geom::aabb2<T> const& aabb)
{
    if (!polyline.is_empty() && aabb.intersects(polyline.aabb()))
    {
        for (size_t i = 0; i + 1 < polyline.size();
             ++i) // iterate over all edges checking for intersection
        {
            if (intersects(aabb, polyline.edge(i)))
            {
                return true;
            }
        }
    ...
    if (!polygon.is_empty() && aabb.intersects(polygon.aabb()))
    {
        ...
        else
        {
            for (size_t i = 0; i < polygon.size();
                 ++i) // iterate over all edges checking for intersection
            {
                if (intersects(aabb, polygon.edge(i)))
                {
                    return true;
                }
            }
...
bool intersects(geom::polygon<T> const& polygon, geom::segment2<T> const& segment)
{
    if (!polygon.is_empty() && polygon.aabb().intersects(segment.aabb()))
    {
        ...
        if (polygon.contains(segment.a, boundary_types::closed) ||
            polygon.contains(segment.b, boundary_types::closed))
        {
            return true;
        }
        else
        {
            for (size_t i = 0; i < polygon.size();
                 ++i) // iterate over all edges checking for intersection
            {
                if (intersects(segment, polygon.edge(i)))
                {
                    return true;
                }
            }
...
bool intersects(geom::polygon<T> const& polygon, geom::polyline2<T> const& polyline)
{
    if (!polygon.is_empty() && !polyline.is_empty() && polygon.aabb().intersects(polyline.aabb()))
    {
        for (size_t i = 0; i + 1 < polyline.size();
             ++i) // iterate over all edges checking for intersection
        {
            if (intersects(polygon, polyline.edge(i)))
            {
                return true;
            }
        }
...
bool intersects(geom::ray3<T> const& ray, geom::plane<T> const& plane)
{
    ...
    if (signed_dist == math::constants<T>::zero) // if the origin of the ray is on the plane, then
                                                 // they certainly intersect
    {
        return true;
    }
    else if (dot == math::constants<T>::zero) // if the normal and the ray are orthogonal, they do
                                              // not intersect
    {
        return false;
    }
    else
    {
        ...
        // plane. this means that both signed_dist and dot are non-zero. if the signs of those
        // values are opposite, then the ray and the plane intersect
 ...
 * @tparam T Number type (eg float)
 * @note This function reports false positives for some inputs (returns true for inputs that do not
 * actually intersect)
 * @param [in] frustum
 * @param [in] aabb
