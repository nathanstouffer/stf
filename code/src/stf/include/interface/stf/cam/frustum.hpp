...
class frustum final
{
  ...
  public:
    ...
    bool contains(vec_t const& point) const
    {
        ...
        {
            if (m_planes[i].side(point) < math::constants<T>::zero)
            {
                return false;
            }
        }
    ...
    bool contains(aabb_t const& aabb) const
    {
        for (size_t i = 0; i < c_num_planes; ++i)
        {
            // check if the extremity in the direction of the anti-normal is contained in the
            // halfspace
     ...
     * @param [in] aabb The query aabb
     * @note This algorithm identifies false positives (returns true for some frustum/aabb that
    ...
    bool intersects_fast(aabb_t const& aabb) const
    {
        if (m_aabb.intersects(aabb))
        {
            ...
            for (size_t i = 0; i < c_num_planes; ++i)
            {
                // check if the extremity in the direction of the normal is contained in the
                // halfspace
                plane_t const& plane = m_planes[i];
                vec_t extremity = aabb.extremity(plane.normal());
                bool contained = plane.side(extremity) >= math::constants<T>::zero;
            ...
            // check if any box plane entirely excludes the frustum
            {
                std::array<vec_t, 8> verts = {
                    // near points
                    m_vertices.ntl, m_vertices.ntr, m_vertices.nbl, m_vertices.nbr,
                    // far points
                    m_vertices.ftl, m_vertices.ftr, m_vertices.fbl, m_vertices.fbr};

                for (size_t d = 0; d < 3; ++d) // iterate over each dimension (x, y, and z)
                {
                    // check the plane at the minimum of the box that has a normal in the +d
                    // direction
                    {
                    ...
                    // check the plane at the maximum of the box that has a normal in the -d
                    // direction
                    {
                        // count the number of frustum vertices outside the box
                        size_t outside = 0;
            ...
            // if we arrive here, we were not able to prove that the frustum/aabb do not intersect.
            // so we must assume that they do intersect, though this maybe be incorrect (resulting
            // in a false positive)
            return true; // fallthrough to return true
        }
        else
    ...
    {
        if (intersects_fast(aabb)) // if the fast check returns true, do a more thorough check to
                                   // avoid false positives
        {
            // m_canonical_edge_axes contains only the axes defined the cross product of pairs of
            // edges -- the face normals are covered by intersects_fast
    ...
    struct vertices final
    {
        ...
        explicit vertices(scamera<T> const& camera)
        {
            ...
            // compute points on the near plane
            {
                ...
                ntl = center + delta_up - delta_right;
                ntr = center + delta_up + delta_right;
                nbl = center - delta_up - delta_right;
                nbr = center - delta_up + delta_right;
            }
            ...
            // compute points on the far plane
            {
                ...
                ftl = center + delta_up - delta_right;
                ftr = center + delta_up + delta_right;
                fbl = center - delta_up - delta_right;
                fbr = center - delta_up + delta_right;
            }
        }
    };
  ...
  private:
    static std::array<vec_t, 18> edge_axes(basis_t const& basis, vertices const& verts)
    {
        ...
        // add axes for cross products between pairs of edges from each polyhedra (the box normals
        // and edge directions are the same)
    ...
    static std::array<vec_t, 26> all_axes(basis_t const& basis,
                                          std::array<plane_t, c_num_planes> const& planes,
  ...
  private:
    explicit frustum(vertices const& verts) : m_vertices(verts)
    {
        ...
        m_planes[nearp] = geom::plane<T>(
            verts.ntl, -m_planes[farp].normal()); // define near plane as a function of the far
                                                  // plane to avoid precision issues
        ...
        std::vector<vec_t> points = {
            // near points          // far points
            verts.ntl, verts.ntr, verts.ftl, verts.ftr, verts.nbl, verts.nbr, verts.fbl, verts.fbr,
        };
  ...
  private:
    ...
    aabb_t m_aabb;
    std::array<vec_t, 18>
        m_canonical_edge_axes; // additional separation axes to test when intersecting with an aabb
};
