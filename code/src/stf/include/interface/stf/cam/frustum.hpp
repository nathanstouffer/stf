...
public:
     ...
     * @param [in] aabb The query aabb
     * @note This algorithm may return false positives (returns true for some frustum/aabb that don't actually
     * intersect)
    ...
    bool intersects(aabb_t const& aabb) const
    {
        ...
        {
            // m_canonical_edge_axes contains only the axes defined the cross product of pairs of edges -- the face
            // normals are covered by intersects_fast
...
private:
    static std::array<vec_t, 18> edge_axes(basis_t const& basis, vertices const& verts)
    {
        ...
        // add axes for cross products between pairs of edges from each polyhedra (the box normals and edge directions
        // are the same)
    ...
    static std::array<vec_t, 26> all_axes(basis_t const& basis, std::array<plane_t, c_num_planes> const& planes,
