...
/**
 * @file scamera.hpp
 * @brief A file containing a class that represents a simple camera defined by a point in R^3 and a look direction in
...
public:
     ...
     */
    scamera(vec_t const& _eye, T const _theta, T const _phi, T const _nearp, T const _farp, T const _aspect,
            T const _fov)
        : eye(_eye)
        , theta(_theta)
        , phi(_phi)
...
{
    return equ(lhs.eye, rhs.eye, eps) && math::equ(lhs.theta, rhs.theta, eps) && math::equ(lhs.phi, rhs.phi, eps) &&
           math::equ(lhs.nearp, rhs.nearp, eps) && math::equ(lhs.farp, rhs.farp, eps) &&
           math::equ(lhs.aspect, rhs.aspect, eps) && math::equ(lhs.fov, rhs.fov, eps);
}
