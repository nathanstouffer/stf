...
/**
 * @file scamera.hpp
 * @brief A file containing a class that represents a simple camera defined by a point in R^3 and a
 * look direction in spherical coordinates
 */
...
/**
 * @brief Struct to represent a simple camera -- one where the right vector is always in the plane
 * normal to (0, 0, 1). The look direction is represented by spherical coordinates with the eye as
 * the origin
...
struct scamera final
{
  ...
  public:
     ...
     */
    scamera(vec_t const& _eye, T const _theta, T const _phi, T const _nearp, T const _farp,
            T const _aspect, T const _fov)
        : eye(_eye)
        , theta(_theta)
        , phi(_phi)
        , nearp(_nearp)
     ...
     * @param [in] _phi
     */
    scamera(vec_t const& _eye, T const _theta, T const _phi)
        : scamera(_eye, _theta, _phi, c_default_nearp, c_default_farp)
    {
    }
     ...
     */
    scamera(vec_t const& _eye, T const _theta, T const _phi, T const _nearp, T const _farp)
        : scamera(_eye, _theta, _phi, _nearp, _farp, c_default_aspect)
    {
    }
    ...
    scamera(vec_t const& _eye, T const _theta, T const _phi, T const _nearp, T const _farp,
            T const _aspect)
        : scamera(_eye, _theta, _phi, _nearp, _farp, _aspect, c_default_fov)
    {
    }
...
{
    return equ(lhs.eye, rhs.eye, eps) && math::equ(lhs.theta, rhs.theta, eps) &&
           math::equ(lhs.phi, rhs.phi, eps) && math::equ(lhs.nearp, rhs.nearp, eps) &&
           math::equ(lhs.farp, rhs.farp, eps) && math::equ(lhs.aspect, rhs.aspect, eps) &&
 ...
 */
template <typename T>
inline scamera<T> orbit(scamera<T> const& camera, math::vec3<T> const& focus, T const delta_phi,
                        T const delta_theta)
{
