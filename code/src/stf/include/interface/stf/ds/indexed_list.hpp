 ...
 * @brief A file containing a class that implements a list with a keyed index overlaid on top for efficient access
 */
...
public:
     ...
     */
    inline void push_front(key_t const& key, entry_t entry) { insert_or_assign(m_list.begin(), key, std::move(entry)); }
     ...
     * @param [in] entry The entry to be stored
     */
    inline void push_back(key_t const& key, entry_t entry) { insert_or_assign(m_list.end(), key, std::move(entry)); }
